...
public class GPSActivity extends Activity implements SharedConstants
{
	...
	//массивы для MoonSunMetods
	static String sunrise_sunset[];
	static String twilight_civil[];
	static String twilightNautical[];
	static String twilightAstronomical[];
	static String twilighGoldenHour[];
	static String twilighBlueHour[];
	static String twilighNightHour;
	static String sunNoonNadir[];
	static double otherSunParametrs[];
	static String moonriseMoonset[];
	static double otherMoonParametrs[];
	//массивы для MoonSunMetods
	//четвертая страница
			...
			case 3:
				if(!page3_is_Null()){//если ни один обьект четвертой страницы пейджера, не ссылается на null
					if(!LocationData.locationIsNull()){//если уже инициализировались переменные
						...
						es.execute(worker);//получаем массивы для MoonSunMetods
	...
	//в дочернем потоке, потому что подвисал основной поток
	//получает массивы для MoonSunMetods
	static class Worker implements Runnable
	{
		...
		public void run()
		{
			// TODO: Implement this method
			//восход/заход
			sunrise_sunset = MoonSunMetods.getSunriseSunset(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся четырехэлементный массив 
			//гражданские сумерки
			twilight_civil = MoonSunMetods.getTwilightCivil(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//морские сумерки
			twilightNautical = MoonSunMetods.getTwilightNautical(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//астрономические сумерки
			twilightAstronomical = MoonSunMetods.getTwilightAstronomical(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//золотой час
			twilighGoldenHour= MoonSunMetods.getTwilighGoldenHour(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//синий час
			twilighBlueHour = MoonSunMetods.getTwilighBlueHour(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//ночной час
			twilighNightHour = MoonSunMetods.getTwilighNightHour(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);
			//noon/nadir солнца
			sunNoonNadir = MoonSunMetods.getSunNoonNadir(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//
			otherSunParametrs = MoonSunMetods.getOtherSunParametrs(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся четырехэлементный массив 
			//ЛУНА
			moonriseMoonset = MoonSunMetods.getMoonriseMoonset(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся двуэлементный массив 
			//
			otherMoonParametrs = MoonSunMetods.getOtherMoonParametrs(LocationData.latitudeGPS,LocationData.longitudeGPS,LocationData.altitudeGPS);//вернулся трехэлементный массив 
			//setText:
