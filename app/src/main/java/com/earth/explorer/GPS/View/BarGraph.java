...
public class BarGraph extends View
{
	...
	public void init(){
		...
		//provider = Settings.Secure.getString(MainActivity.context. getContentResolver(), Settings.Secure.LOCATION_PROVIDERS_ALLOWED);
		paint = new Paint();
		...
		paintFix = new Paint();
		paintFix.setColor(Color.parseColor("#64B5F6"));//был #448AFF
	...
	public void drawBarGraph(Canvas canvas){
		...
		if(MyService. gnssStatusCallback.arraySat!=null){//if(MainActivity.arraySat!=null){



			int count=MyService. gnssStatusCallback.arraySat.size();
			...
			if((xLength>=stopLefX)&(xLength<=stopRightX)){
				...
				for (MySatellite s : MyService.gnssStatusCallback. arraySat) {//цикл11
				...
				if(xLength<=stopLefX){//если ушли левее допустимого
					...
					for (MySatellite s : MyService.gnssStatusCallback. arraySat) {
	...
	{
		super.onDraw(canvas);
		//drawBarGraph(canvas);//рисуем
		
		
		if(GnssStatusCallback.count!=0){
			drawBarGraph(canvas);//рисуем
		}
		...
		/*
		if(GPSActivity.provider.equals("")&(MyService.serviceBoolean)){
			canvas.drawColor(Color.parseColor("#202125"));
		}else{

			//внутреннее ветвление
			if(MyService.serviceBoolean){//переменная связана с switch
				if(GnssStatusCallback.count!=0){
					drawBarGraph(canvas);//рисуем
				}
			}else{
				canvas.drawColor(Color.parseColor("#202125"));//не рисуем
			}
			//внутреннее ветвление
