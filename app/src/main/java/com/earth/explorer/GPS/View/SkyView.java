...
public class SkyView extends View implements SharedConstants
{
	...
	usedInFixPaintShow, fillPaintShow;
	...
	public void init(Context context)
	{
		...
		fillPaintShow = new Paint();
        fillPaintShow.setColor(Color.parseColor("#448AFF"));
        fillPaintShow.setStyle(Paint.Style.FILL);
        fillPaintShow.setAntiAlias(true);
		...
		usedInFixPaintShow = new Paint();
        usedInFixPaintShow.setColor(Color.parseColor("#448AFF"));//2d84ff
		usedInFixPaintShow.setShadowLayer(2,2,2,Color.parseColor("#101010"));
        usedInFixPaintShow.setStyle(Paint.Style.FILL);
        usedInFixPaintShow.setAntiAlias(true);
	...
	private void drawSatellite(Canvas c, int minScreenDimen, float elev, float azim, float cn0, int prn,
		...
		//рисуем спутник, в обычном режиме:
...
/////
		...
		if(GnssStatusCallback. arraySat!=null){
			...
			for (MySatellite s : GnssStatusCallback. arraySat) {
