...
public class MoonSunMetods
{
	...
	public static synchronized String[] getSunriseSunset(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getTwilightCivil(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getTwilightNautical(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getTwilightAstronomical(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getTwilighGoldenHour(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getTwilighBlueHour(double latitude, double longitude, double altitude){
	...
	public static synchronized String getTwilighNightHour(double latitude, double longitude, double altitude){
	...
	public static synchronized String[] getSunNoonNadir(double latitude, double longitude, double altitude){
	...
	public static synchronized double[] getOtherSunParametrs(double latitude, double longitude, double altitude){
	...
	//восход/заход Солнца
	//возвращает двуэлементный строковый массив. 
	//первый элемент:время восхода луна
	//второй элемент:время захода луны
	public static synchronized String[] getMoonriseMoonset(double latitude, double longitude, double altitude){

		String moonriseMoonset[]=new String[2];
		Date date=new Date();//по умолчанию сразу инициализируемся текущим временем
		
		MoonTimes moonTimes;
		MoonTimes moonTimesFull;
		MoonTimes.Parameters param;
		MoonTimes.Parameters paramFull;
		String  str="";
		param= MoonTimes.compute();
		paramFull = MoonTimes.compute();
		...
		//теперь выполню рассчет
		moonTimes=param.execute();//создается обьект класса содержащий рассчет
		moonTimesFull = paramFull.execute();//создается обьект класса содержащий рассчет
		//извлекается рассчет так:
		//moonTimes.getRise();//для восхода
		//moonTimes.getSet();//для захода
		//возвращаются экземпляры класса Date
		//при этом может вернуться null
		...
		//ВОСХОД ЛУНЫ
		//а мы не только извлекаем рассчет, но и сразу вписываем в строки массива, строчки с информацией:
		if(moonTimes.getRise()!=null){//если луна взойдет.
			str=// "Восход: "+
				//"Восход Луны: "+
				detailsSimpleDateFormat.format(moonTimes.getRise());
			    moonriseMoonset[0]=str;//вот, инициализировал строку массива
		}else{//иначе луна сегодня не взойдет

			

			//и заодно выясню ближайшее время когда луна взойдет
			//но они возможно тоже могут вернуть нам null, поэтому предостережемся от сбоя
			if(moonTimesFull.getRise()!=null){
				str=// "Восход: "+
					//isalways+"\n"+
					//">>Ближайшее>>"+
					//"Восход Луны: "+
					detailsSimpleDateFormat.format(moonTimesFull.getRise());
				    moonriseMoonset[0]=str;//вот, инициализировал строку массива
			}else{
				str="нет данных";//пишем что вернулся null
				moonriseMoonset[0]=str;
			}
		}
		//ВОСХОД ЛУНЫ

		//ЗАХОД ЛУНЫ
		if(moonTimes.getSet()!=null){//если Луна зайдет
			str=// "Заход: "+
				//"Заход Луны: "+
				detailsSimpleDateFormat.format(moonTimes.getSet());
			    moonriseMoonset[1]=str;
		}else{//иначе Луна в этот день не зайдет

			//и заодно выясню ближайшее время когда Луна зайдет
			//но они возможно тоже могут вернуть нам null, поэтому предостережемся от сбоя
			if(moonTimesFull.getSet()!=null){
				str=// "Заход: "+
					//isalways+"\n"+
					//">>Ближайшее>>"+
					//"Заход Луны: "+
					detailsSimpleDateFormat.format(moonTimesFull.getSet());
				    moonriseMoonset[1]=str;
			}else{
				str="нет данных";//пишем что вернулся null
				moonriseMoonset[1]=str;
		...
		return moonriseMoonset;
		}
	
		
		
		
	//возвращает трехэлементый массив double. 
	//первый элемент:высота луны над горизонтом в градусах.
	//второй элемент:азимут луны в градусах по северу
	//третий элемент:расстояние до луны в километрах
	public static synchronized double[] getOtherMoonParametrs(double latitude, double longitude, double altitude){
		
		double moonParam[]=new double[3];
		Date date=new Date();//по умолчанию сразу инициализируемся текущим временем


		//ВЫСОТА ЛУНЫ НАД ГОРИЗОНТОМ В ГРАДУСАХ, АЗИМУТ, И РАССТОЯНИЕ
		MoonPosition.Parameters params = MoonPosition.compute();
		params.on(date);
		params.at(latitude, longitude)
			.height(altitude);//считаем всё с высотой.

		//теперь выполню рассчет
		MoonPosition pos = params.execute();

		double altitudeMoon =  pos.getAltitude();//высота Луны над горизонтом в градусах
		double azimuthMoon = pos.getAzimuth();//азимут Луны в градусах по северу
		double distanceMoon = pos.getDistance();//расстояние до Луны в километрах

		//ВЫСОТА ЛУНЫ НАД ГОРИЗОНТОМ В ГРАДУСАХ, АЗИМУТ, И РАССТОЯНИЕ
		moonParam[0]=altitudeMoon;
		moonParam[1]=azimuthMoon;
		moonParam[2]=distanceMoon;
