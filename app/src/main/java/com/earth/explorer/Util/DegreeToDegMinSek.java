...
public class DegreeToDegMinSek
{
	...
	private static synchronized int degree(double mdouble){
		String str=mdouble+"";
	...
	private static synchronized int minut(double degree){
	...
	private static synchronized int sekond(double degree){
		double minut= дробная_часть(degree)*60;
		double sekond=дробная_часть(minut)*60;
		//return (int)round(sekond,2);
		return (int)sekond;
	}
...
//ВОЗВРАЩАЕМ СТРОКУ ГРАДУСЫ°МИНУТЫ'СЕКУНДЫ"
	...
	public static synchronized String latDegMinSek(double degree){
...
//ВОЗВРАЩАЕМ СТРОКУ ГРАДУСЫ°МИНУТЫ'СЕКУНДЫ"
	//Метод на вход принимает градусы, возвращает строку
	//число полных градусов, число полных минут, число полных секунд.
	public static synchronized String lonDegMinSek(double degree){
	...
	//дробная часть числа
	public static synchronized double дробная_часть(double mdouble){
	...
	//принимает на вход градусы, возвращает градусы, минуты, секунды
	public static synchronized String getDegMinSek(double degree){
		
		String znak=(degree<0)?"-":"";
		
		String deg= znak+ degree(degree)+"° "+minut(degree)+"' "+sekond(degree)+"\"";
		return deg;
	};
